...
class JurisdictionResult:
    level: int  # 0-4 (aligned with DICTU/EU SEAL framework)
    label: str
...
def classify_jurisdiction(
    ...
    # Level 4: parent in EU AND server in EU AND peeringdb org in EU
    ...
    # Level 3: parent in EU AND server in EU (peeringdb unknown is OK)
    ...
    # Level 3 (alt): no parent from manual mapping, but PeeringDB confirms EU organization
    ...
    # Level 2: server in EU AND parent unknown AND peeringdb unknown
    # Important: do NOT catch non-EU parents here — those should fall through to level 1
    ...
    # Level 2 (adequate): parent in adequate country (GB, JP, etc.) and server in EU
    ...
    # Level 1: non-EU parent (server in EU or not)
    if parent_is_non_eu:
        if server_in_eu:
            reasons.append(
